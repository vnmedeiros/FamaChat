...
/**
 * Fetching chat room including all the chat messages by user
 *  */
$app->get('/chat_rooms/:id/user', function($user_id) {
    global $app;
    $db = new DbHandler();

    $result = $db->getChatRoomByUser($user_id);
    ...
    while ($chat_room = $result->fetch_assoc()) {
        ...
        $tmp["name"] = $chat_room["name"];
        $tmp["created_at"] = $chat_room["created_at"];
        $tmp["is_admin"] = $chat_room["is_admin"];
        array_push($response["chat_rooms"], $tmp);
    }
 ...
 * Fetching single chat room including all the chat messages
 *  */
$app->get('/private_chats/:id/user', function($user_from_id) {
    global $app;
    $db = new DbHandler();

    $result = $db->getPrivateChats($user_from_id);
    ...
    $last_user_id = 0;
    // looping through result and preparing tasks array
    while ($privates = $result->fetch_assoc()) {
        if ($privates['user_to_id'] != NULL) {
            // message node
            $user = array();
            $user["created_at"] = $privates["created_at_user_to"];
            $user["user_to_id"] = $privates["user_to_id"];
            $user["name"] = $privates["name_user_to"];
 ...
 * Fetching single chat room including all the chat messages
 *  */
$app->get('/private_chats/from/:id_from/to/:id_to/message', function($user_from_id, $user_to_id) {
    global $app;
    $db = new DbHandler();

    $result = $db->getPrivateChatsMenssages($user_from_id, $user_to_id);
    ...
    while ($chat_room = $result->fetch_assoc()) {
        ...
        $cmt["message_id"] = $chat_room["message_id"];
        $cmt["created_at"] = $chat_room["created_at"];
        $cmt['user_from_id'] = $chat_room['user_from_id'];
        $cmt['user_to_id'] = $chat_room['user_to_id'];
...
// Add user to chat user-user
$app->post('/chat_rooms/add/user', function() use ($app) {
    // check for required params
    verifyRequiredParams(array('user_id_from', 'user_email_to', 'chat_room_id'));
    
    // reading post params
    $user_id_from = $app->request->post('user_id_from');
    $chat_room_id = $app->request->post('chat_room_id');
    $user_email_to = $app->request->post('user_email_to');

    // validating email address
    validateEmail($user_email_to);

    $db = new DbHandler();
    $response = $db->addUserToChat($user_id_from, $user_email_to, $chat_room_id);
    ...
    if ($response['error'] == false) {
        ...
        $user = $db->getUser($user_id_from);

        $data = array();
        $data['user'] = $user;
        $data['message'] = $response['message'];
        $data['chat_room_id'] = $chat_room_id;
        $data['new_member'] = true;
        $data['image'] = '';
 ...
 * * */
$app->post('/private_chats/:id/message', function($to_user_id) {
    global $app;
    $db = new DbHandler();
...
$app->post('/users/push_test', function() {
    ...
    if ($image == 'true') {
        $data['image'] = 'https://scontent.fgyn2-1.fna.fbcdn.net/v/t1.0-9/395739_348382681856651_1971765626_n.jpg?oh=960c3a7ba331f433f4642b7ca8c00a88&oe=59EF869D';
    } else {
        $data['image'] = '';
    }
